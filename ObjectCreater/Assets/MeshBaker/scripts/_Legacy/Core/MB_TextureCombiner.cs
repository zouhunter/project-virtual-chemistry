...
namespace DigitalOpus.MB.Core{	
	...
	public class MB_TextureCombiner{
		...
		public bool fixOutOfBoundsUVs = false;
		public int maxTilingBakeSize = 1024;
		public bool saveAtlasesAsAssets = false;
		public MB2_PackingAlgorithmEnum packingAlgorithm = MB2_PackingAlgorithmEnum.UnitysPackTextures;
		...
		bool _collectPropertyNames(Material resultMaterial, List<string> texPropertyNames){
			...
			for (int i = 0; i < customShaderPropNames.Count; i++){
				if (m.HasProperty(customShaderPropNames[i]) ){
					...
					if (LOG_LEVEL >= MB2_LogLevel.warn) Debug.LogWarning("Result material shader does not use property " + customShaderPropNames[i] + " in the list of custom shader property names");	
				}
			}			
				...
				for(int i = 0; i < distinctMaterialTextures.Count; i++){
					...
					for (int j = 0; j < txs.ts.Length; j++){
						...
						if (!matTex.scale.Equals(Vector2.one) && distinctMaterialTextures.Count > 1){
							if (LOG_LEVEL >= MB2_LogLevel.warn) Debug.LogWarning("Texture " + matTex.t + "is tiled by " + matTex.scale + " tiling will be baked into a texture with maxSize:" + maxTilingBakeSize);
						}
						if (!matTex.obUVscale.Equals(Vector2.one) && distinctMaterialTextures.Count > 1 && fixOutOfBoundsUVs){
							if (LOG_LEVEL >= MB2_LogLevel.warn) Debug.LogWarning("Texture " + matTex.t + "has out of bounds UVs that effectively tile by " + matTex.obUVscale + " tiling will be baked into a texture with maxSize:" + maxTilingBakeSize);
						}	
						if (matTex.t != null){
...
//			}
		...
		Vector2 getAdjustedForScaleAndOffset2Dimensions(MeshBakerMaterialTexture source){
			...
			if (newWidth > maxTilingBakeSize) newWidth = maxTilingBakeSize;
			if (newHeight > maxTilingBakeSize) newHeight = maxTilingBakeSize;
